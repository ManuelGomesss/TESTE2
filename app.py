...
from flask_login import LoginManager, current_user, login_user, logout_user, login_required as flask_login_required, UserMixin
...
admin.add_view(SecureModelView(User, session.bind, name="Utilizadores"))
...
# --- Função auxiliar de rotas protegidas ---
    ...
    if request.method == 'POST':
        ...
        for sheet in xls.sheet_names:
            ...
            if all(col in tmp.columns for col in required):
                ...
                break
        if df is None:
            return render_template('import_6ano.html', error='Não encontrei UC, ECTS UC e Avaliação Nota em nenhuma folha.')
        df = df[~(
            df['UC'].astype(str).str.startswith('Opcional', na=False) &
            df['Avaliação Nota'].isna()
        )]
        records = df.to_dict('records')
        session['import_df'] = records
